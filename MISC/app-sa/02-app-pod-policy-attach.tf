resource "aws_iam_policy" "app_pod_secrets_policy" {
  name        = "${local.resource_name}-eso-secrets-policy"
  description = "Allow app pod to read /${var.project}/${var.env} secrets from AWS Secrets Manager"
...
resource "aws_iam_role_policy_attachment" "app_pod_secrets_policy_attachment" {
  role       = aws_iam_role.app_pod_role.name
  policy_arn = aws_iam_policy.app_pod_secrets_policy.arn
}
